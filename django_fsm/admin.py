...
class FSMTransitionMixin(ModelAdmin[fsm._FSMModel]):
    ...
    def get_fsm_object_transitions(
        ...
        return list(self._get_fsm_object_transitions(request=request, obj=obj))
    ...
    def _handle_fsm_transition(
        ...
        # Validate transition method exists and is callable
        if not self._is_valid_transition_method(obj, transition_name):
            self.message_user(
                request=request,
                message=self.fsm_transition_not_valid_msg.format(transition_name=transition_name),
        ...
        if self.get_fsm_transition_custom(instance=obj, transition_func=transition_func).get(
            "form"
        ):
            return redirect(
                reverse(
        ...
        return self._execute_fsm_transition(request, obj, transition_func, transition_name)

    def _is_valid_transition_method(self, obj: typing.Any, transition_name: str) -> bool:
        ...
        if not hasattr(transition_func, "_django_fsm"):
            logger.warning("Attempted to call non-FSM method %s", transition_name)
            return False
    ...
    def fsm_transition_view(
        ...
        if not self._is_valid_transition_method(obj, transition_name):
            logger.warning("Invalid transition method %s attempted", transition_name)
            return HttpResponseBadRequest(f"{transition_name} is not a valid transition method")

        transition_method = getattr(obj, transition_name)
        transition = self._get_transition_from_method(transition_method)

        if not transition:
            logger.warning("No transition found for method %s", transition_name)
            return HttpResponseBadRequest(
                f"No transition configuration found for {transition_name}"
            )

        if TransitionForm := self.get_fsm_transition_form(transition):  # noqa: N806
            return self._handle_form_transition(
                request, obj, transition, transition_method, TransitionForm, transition_name
            )

        return self._handle_direct_transition(request, obj, transition_method, transition_name)
    ...
    def _handle_form_transition(
        ...
        transition_form_class: type[Form | ModelForm[typing.Any]],
        transition_name: str,
    ) -> HttpResponse:
        """Handle FSM transition that requires form input."""
        form_factory: typing.Callable[..., Form] | typing.Callable[..., ModelForm[typing.Any]]
        if issubclass(transition_form_class, ModelForm):
            form_factory = partial(transition_form_class, instance=obj)
        else:
            form_factory = transition_form_class

        if request.method == "POST":
            transition_form = form_factory(data=request.POST)
            if transition_form.is_valid():
                try:
                    ...
                    return self._get_transition_redirect(obj)
                except Exception as e:
    ...
    def _handle_direct_transition(
        ...
        """Handle FSM transition without form input."""
        try:
            transition_method()
            obj.save()
            self.message_user(
            ...
            logger.exception("Direct transition %s failed", transition_name)
            self.message_user(
                request,
                self.fsm_transition_error_msg.format(transition_name=transition_name, error=str(e)),
                messages.ERROR,
            )

        return self._get_transition_redirect(obj)

    def _get_transition_redirect(self, obj: typing.Any) -> HttpResponseRedirect:
        """Get redirect response after transition execution."""
        return redirect(
