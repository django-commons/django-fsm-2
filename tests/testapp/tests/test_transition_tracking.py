...
class TransitionTrackingTests(TestCase):
    def test_default_tracking_uses_generic_log(self) -> None:
        ...
        post.publish(author=user, description="published via generic log")

        log = StateLog.objects.get(
            content_type=ContentType.objects.get_for_model(GenericTrackedPost),
    ...
    def test_custom_tracking_writes_to_model_log(self) -> None:
        ...
        post.publish(author=user, description="published via custom log")
